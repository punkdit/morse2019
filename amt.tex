...
are chain maps, ie.
\begin{align*}
    \partial^\MM\circ f &= f\circ \partial \\
    \partial\circ g &= g\circ \partial^\MM.
\end{align*}
...
chain homotopies $\chi:g\circ f\to I_C$ and $0:f\circ g\to I_{C^\MM}$.
This means we have
\begin{align*}
    g_i \circ f_i - I_C &= \partial_{i+1} \chi_i + \chi_{i-1}\partial_i,\\
    f_i \circ g_i - I_{C^\MM} &= 0.
\end{align*}
...
{\bf Example 1.}
A planar graph, with six vertices, $|X_0|=6$
and seven edges, $|X_1|=7$.

\begin{center}
\includegraphics[]{images/pic-complex-0}
\ \ \ \ \ \includegraphics[]{images/pic-poset-0}
\end{center}
